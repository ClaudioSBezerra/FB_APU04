...
**Current focus:** Phase 12 — tela-compara-o-fiscal-navega-o
...
- 2026-07-03: Pós-milestone v6.00 — sessão de correções/iteração no módulo Teste Pacote Fiscal com o usuário testando ao vivo em produção (sem plano GSD formal, commits diretos):
  1. `b7c9975` fix: item de nav "Teste Pacote Fiscal" faltava em AppRail.tsx (Plan 12-03 editou AppSidebar.tsx, que é código morto — ver skill `fb-apu04-nav-architecture` e memória `frontend-nav-apprail-vs-appsidebar`)
  2. `ebc187c`/`402130a`/`2206d67` feat: filtro de data no combobox → depois substituído por `NfeSearchList.tsx` (lista com checkbox + "Executar Selecionadas" em lote) → colunas Nº Nota/Cliente/Item + campo Base IBS/CBS → Resumo da Nota (acumulado dos itens vs. total do cabeçalho) + scroll automático ao selecionar nota
  3. `91eb1d7` **decisão revertida**: módulo Teste Pacote Fiscal deixou de usar `nfe_saidas`/`nfe_saidas_itens` — agora tem pipeline de import de XML e tabelas isoladas (`pacotefiscal_nfe_saidas`/`_itens`, migration 148, cabeçalho completo emit/dest). Ver memória `pacotefiscal-pipeline-isolado` e PROJECT.md Key Decisions. Migration validada e2e contra Postgres descartável antes do deploy.
  4. `7b337e8` feat: removida exigência de 3 caracteres na busca (lista as 50 mais recentes sem filtro); + filtros UF Origem/UF Destino/Cliente/Emitente
  5. `a48fcd0` feat: painel de debug pós-execução (conexão Oracle, lookup grupo fiscal, chamada do pacote, por item) — trace, não streaming ao vivo
  6. `a949b0d` fix: `lookupGrupoFiscal` removia dígito verificador do código do produto antes de buscar em PROD/PRODB (XML "3796949" → tabela "379694") — confirmado pelo usuário com produto real
  - **Pendente para próxima sessão:** confirmar deploy de `a949b0d` (verificar logs do backend por "Executing migration" NÃO se aplica aqui — é fix de código, não migration); usuário pediu para encerrar o dia após este ajuste. Ver também item já conhecido: bloco IBS/CBS por item no parser XML é melhor esforço, não validado contra XML real de Reforma Tributária.
