...
const (
	BackendVersion = "2.5.1"
	FeatureSet     = "Escrituração de Entradas EFD, Importação ERP Bridge, NF-e Entradas, CT-e Entradas, Enriquecimento PIS/COFINS/IPI, Malha Fina, Apuração IBS/CBS, Créditos em Risco, SPED layout 020, Módulo Teste Pacote Fiscal, Personas por Módulo, Simulação IBS/CBS na Base do ICMS"
)
...
func main() {
	...
	http.HandleFunc("/api/admin/personas", withAuth(handlers.ListPersonasHandler, "admin"))
	http.HandleFunc("/api/admin/personas/update", withAuth(handlers.UpdatePersonaHandler, "admin"))
	...
	http.HandleFunc("/api/fiscal/oracle-info", withAuth(handlers.FiscalOracleInfoHandler, ""))
	...
	http.HandleFunc("/api/icms-fronteira/recalcular", func(w http.ResponseWriter, r *http.Request) {
		database := getDB()
		if database == nil {
			jsonServiceUnavailable(w)
			return
		}
		handlers.AuthMiddleware(handlers.IcmsFronteiraRecalcularHandler(database), "")(w, r)
	})
	http.HandleFunc("/api/icms-fronteira/fretes-pendentes", func(w http.ResponseWriter, r *http.Request) {
		database := getDB()
		if database == nil {
			jsonServiceUnavailable(w)
			return
		}
		handlers.AuthMiddleware(handlers.IcmsFronteiraFretesPendentesHandler(database), "")(w, r)
	})
	http.HandleFunc("/api/icms-fronteira/fretes-pendentes/exportar/xlsx", func(w http.ResponseWriter, r *http.Request) {
		database := getDB()
		if database == nil {
			jsonServiceUnavailable(w)
			return
	...
	http.HandleFunc("/api/icms-fronteira/st-itens/exportar/xlsx", func(w http.ResponseWriter, r *http.Request) {
		database := getDB()
		if database == nil {
			jsonServiceUnavailable(w)
			return
		}
		handlers.AuthMiddleware(handlers.IcmsFronteiraSTItensXLSXHandler(database), "")(w, r)
	})
	http.HandleFunc("/api/icms-fronteira/st-itens/exportar/pdf", func(w http.ResponseWriter, r *http.Request) {
		database := getDB()
		if database == nil {
			jsonServiceUnavailable(w)
			return
	...
	http.HandleFunc("/api/icms-fronteira/periodos", func(w http.ResponseWriter, r *http.Request) {
		database := getDB()
		if database == nil {
			jsonServiceUnavailable(w)
			return
		}
		handlers.AuthMiddleware(handlers.IcmsFronteiraPeriodosHandler(database), "")(w, r)
	})
