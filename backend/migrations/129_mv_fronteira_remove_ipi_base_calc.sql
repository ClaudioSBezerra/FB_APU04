-- 129_mv_fronteira_remove_ipi_base_calc.sql
--
-- Remove IPI da base_calc da antecipação/DIFAL.
-- IPI não integra a BC do ICMS na entrada interestadual:
-- base = v_prod + frete_rat + outro_rat.
-- A coluna ipi_eff permanece na MV para exibição em relatório.
...
SELECT
    ...
    -- base_calc = produto + frete_rat + outro_rat (IPI excluído: não integra BC do ICMS)
    f.sum_item
        + CASE WHEN SUM(f.sum_item) OVER (PARTITION BY f.c100_id) > 0
               THEN f.nf_frete * f.sum_item / SUM(f.sum_item) OVER (PARTITION BY f.c100_id)
               ELSE 0 END
