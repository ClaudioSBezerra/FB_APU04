-- 128_mv_fronteira_icms_inter.sql
--
-- Problema: quando uma NF tem itens com alíquotas interestaduais diferentes
-- (ex.: 12% doméstico + 4% importado no mesmo CFOP), o c190_consol usava
-- MAX(aliq_icms), que descarta a alíquota menor. O crédito interestadual
-- ficava super-estimado → ICMS Devido sub-calculado.
--
-- Fix: adiciona vl_icms_inter = SUM(vl_bc_icms × aliq_icms / 100) por linha
-- do C190. Cada linha tem seu próprio BC e alíquota → crédito ponderado correto.
-- Cap em 12% protege SPEDs que preencheram aliq_icms com a alíquota interna
-- do estado de origem (17-20%) em vez da interestadual.
--
-- Fallback em fronteiraBaseQuery: se vl_icms_inter = 0 (vl_bc_icms NULL em
-- registros antigos), usa v_prod × aliq_icms / 100 como antes.
...
c190_consol AS (
    SELECT id_pai_c100, cfop,
           ...
           -- Crédito correto: BC × alíq por linha (cap 12% = max interestadual).
           -- Preserva mix de alíquotas dentro do mesmo CFOP.
           SUM(
...
fonte AS (
    SELECT
        ...
        -- vl_icms_inter: crédito ponderado por linha C190 (preenchido quando
        -- vl_bc_icms está disponível no SPED; NULL/zero → fallback no query).
