...
--   <COMPANY_ID>  -> uuid da empresa/filial (ex.: '0c3f...-...')
--   <MM/AAAA>     -> período do relatório (ex.: '04/2026')
--
...
WHERE ne.company_id = '<COMPANY_ID>'
  AND ne.data_emissao >= to_date('<MM/AAAA>','MM/YYYY')
  AND ne.data_emissao <  (to_date('<MM/AAAA>','MM/YYYY') + interval '1 month');
...
WHERE j.company_id = '<COMPANY_ID>'
  ...
  AND (j.mes_ano = '<MM/AAAA>'
       OR (j.mes_ano IS NULL
           AND EXTRACT(MONTH FROM j.dt_ini)::int = SPLIT_PART('<MM/AAAA>','/',1)::int
           AND EXTRACT(YEAR  FROM j.dt_ini)::int = SPLIT_PART('<MM/AAAA>','/',2)::int))
